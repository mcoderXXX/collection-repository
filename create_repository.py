...
"""
    ...
    Author: mcoderxxx, date: 12.05.2018
    Log: Add Option --recursive, --verbose and --exclude, add support to respect subfolders
    +++
...
def parse_metadata(metadata_file):
    ...
    try:
        ...
        print('Parse-Error in file: %s' % metadata_file)
        raise e
    ...
    if (addon_metadata.id is None or
        ...
        raise RuntimeError('Invalid addon ID: {}, in file: {}'.format(addon_metadata.id, metadata_file))
    if (addon_metadata.version is None or
        ...
        raise RuntimeError(
            'Invalid addon version: {}, in file: {}'.format(addon_metadata.version, metadata_file))
    return addon_metadata


def generate_checksum(archive_path, is_binary=True, checksum_path_opt=None):
    if args_verbose > 0: print('Generate checksum ...') 
...
def copy_metadata_files(source_folder, addon_target_folder, addon_metadata):
    if args_verbose > 0: print('Copy files ...') 
...
def fetch_addon_from_git(addon_location, target_folder):
    ...
    try:
        ...
        if args_verbose > 0: print('Create addon in target: %s' % addon_metadata.id) 
...
def fetch_addon_from_subfolders(addons_locations, target_folder, dirdeep=0):
    global args_excludelist

    result = []

    # Abort recursion if dirdeep is 0
    if (dirdeep == 0): return result

    currentDir = addons_locations
    metadata_path = os.path.join(currentDir, INFO_BASENAME)

    if args_verbose > 0: print('Enter subfolder: %s' % currentDir)

    # If is a addon directory
    if os.path.exists(metadata_path):
        return fetch_addon_from_folder(addons_locations, target_folder)
    ...
    for dirname in directories:
        ...
        # Skip files and excludelist
        if (( not os.path.isdir(nextDir)) or ( dirname in args_excludelist )): continue


        # Skip target-folder if occurs
        if (os.path.realpath(nextDir) == os.path.realpath(target_folder)): continue

        metadata_path = os.path.join(nextDir, INFO_BASENAME)

        # check if is a addon directory
        if os.path.exists(metadata_path):
            result.append(fetch_addon_from_folder(nextDir, target_folder))
        else:
            result += fetch_addon_from_subfolders(nextDir, target_folder, dirdeep-1)
...
def fetch_addon(addon_location, target_folder, result_slot):
    try:
        if is_url(addon_location):
            ...
            if not args_recursive:
                addon_metadata = fetch_addon_from_folder(
                addon_location, target_folder)
            else:
                #@todo: addon_metadata is list, add to result_slot
                addon_metadata = fetch_addon_from_subfolders(
                addon_location, target_folder, args_recursive)
...
def create_repository(
    ...
    for worker in workers:
        ...
        if type(result.addon_metadata) is list:
            for r in result.addon_metadata:
                metadata.append(r)
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        action='append',
        default=['.svn', '.git'],
        help='Add directory to exclude, you can multiple times set this argument')
    parser.add_argument(
        '--recursive',
        '-r',
        nargs='?',
        ...
        type=int,
        default=None,
        help='Activate support for recursive directories')
    parser.add_argument(
        '--verbose',
        '-v',
        action='count',
        default=0,
        help='Be more verbose, you can use this argument multiple times')
    parser.add_argument(
        'addon',
        nargs='*',
        metavar='ADDON',
